(ns mongodb.main
  "Kotodama WASM entrypoint for the MongoDB clean-room actor (L4) — Clojure port.
  ...
  facts (`mongodb.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mongodb")
(def tier "L4")
...
(def entity-specs
  [{:entity "Cluster"        :plural "clusters"        :id-prefix "mongodb_clu"
    :fields [:name :clusterType :stateName :mongoDBVersion] :required [:name :clusterType]
    :coerce {}                       :refs {}}
   {:entity "DatabaseUser"   :plural "databaseusers"   :id-prefix "mongodb_dat"
    :fields [:username :databaseName :awsIAMType]      :required [:username :databaseName]
    :coerce {}                       :refs {}}
   {:entity "Project"        :plural "projects"        :id-prefix "mongodb_pro"
    :fields [:name :orgId :clusterCount]               :required [:name :clusterCount]
    :coerce {:clusterCount :int}     :refs {}}
   {:entity "Backup"         :plural "backups"         :id-prefix "mongodb_bac"
    :fields [:frequencyType :description]              :required [:frequencyType :description]
    :coerce {}                       :refs {}}
   {:entity "Organization"   :plural "organizations"   :id-prefix "mongodb_org"
    :fields [:orgName :planType :groupId]              :required [:orgName :planType]
    :coerce {}                       :refs {}}
   {:entity "NetworkPeering" :plural "networkpeerings" :id-prefix "mongodb_net"
    :fields [:name :accepterAccountId :localStatus :cloudStatus] :required [:name :localStatus]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"mongodb.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mongodb-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mongodb-compat/ready)
